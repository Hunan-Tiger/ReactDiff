...
class Render(object):
    ...
    def rendering_for_fid(self, path, ind, listener_vectors, speaker_video_clip, listener_reference, listener_video_clip):
        ...
        listener_vectors_device = listener_vectors.get_device()
        listener_vectors[:, :] = torch.from_numpy(kalman_smooth_params(listener_vectors[:, :].cpu().numpy())).to(listener_vectors_device).to(torch.float32)
        #self.faceverse.exp_tensor = listener_vectors[:, :52].view(T, -1).to(listener_vectors.get_device())
        ...
        path_speaker = os.path.join(path, 'fid', 'speaker')
        ...
        for i in range(0, rendered_img_r.shape[0]+1, 25):
            if i==rendered_img_r.shape[0]:
                i=i-1
            cv2.imwrite(os.path.join(path_real, ind+'_'+str(i+1)+'.png'), listener_video_clip[i])
            #cv2.imwrite(os.path.join(path_speaker, ind+'_'+str(i+1)+'.png'), speaker_video_clip[i])
